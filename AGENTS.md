...
- `prepare` also emits periodic plain-text progress lines in non-interactive stdout streams such as Colab `!python ...`, because those subprocess cells can suppress `tqdm` carriage-return redraws even while work is actively progressing.
